...
public class Player extends Component {
    ...
    public void update(double up) {
        ...
        if (this.state != PlayerState.FLYING && !onGround) {
            gameObject.transform.rotation += 10.0f * up;
    ...
    public void draw(Graphics2D graph2) {
        ...
        transform.rotate(gameObject.transform.rotation, width * gameObject.transform.scale.x / 2.0, height * gameObject.transform.scale.y / 2.0);
        transform.scale(gameObject.transform.scale.x, gameObject.transform.scale.y);

        if (state == PlayerState.NORMAL) {
        	graph2.drawImage(layerOne.image, transform, null);
            graph2.drawImage(layerTwo.image, transform, null);
            graph2.drawImage(layerThree.image, transform, null);
        } else {
            transform.setToIdentity();
            transform.translate(gameObject.transform.position.x, gameObject.transform.position.y);
            transform.rotate(gameObject.transform.rotation, width * gameObject.transform.scale.x / 4.0, height * gameObject.transform.scale.y / 4.0);
            ...
            transform.rotate(gameObject.transform.rotation, width * gameObject.transform.scale.x / 2.0, height * gameObject.transform.scale.y / 2.0);
            transform.scale(gameObject.transform.scale.x, gameObject.transform.scale.y);
            graph2.drawImage(spaceship.image, transform, null);
        }
    }
