...
public class LevelEditorControls extends Component {
    private float debounceTime = 0.2f, debounceLeft = 0.0f, debounceKey = 0.2f, debounceKeyLeft = 0.0f, worldX, worldY, dragX, dragY, dragWidth, dragHeight;
    private boolean shiftKeyPressed = false, isEditing = false, wasDragged = false;
    ...
    public void addGameObjectToSelected(Vector2 mousePosition) {
    	mousePosition.x += Window.getScene().camera.position.x;
    	mousePosition.y += Window.getScene().camera.position.y;
        for (GameObject go : Window.getScene().getAllGameObjects()) {
            Bounds bounds = go.getComponent(Bounds.class);
            if (bounds != null && bounds.raycast(mousePosition)) {
    ...
    public List<GameObject> boxCast(float x, float y, float width, float height) {
        ...
        for (GameObject go : Window.getScene().getAllGameObjects()) {
            Bounds bounds = go.getComponent(Bounds.class);
            if (bounds != null) {
                if (go.transform.position.x + bounds.getWidth() <= x0 + width && go.transform.position.y + bounds.getHeight() <= y0 + height && go.transform.position.x >= x0 && go.transform.position.y >= y0) {
                    objs.add(go);
                }
            }
        }
    ...
    public void rotateObjects(float degrees) {
        for (GameObject go : selectedObjects) {
            ...
            TriangleBounds bounds = go.getComponent(TriangleBounds.class);
            if (bounds != null) {
            	bounds.calculateTransform();
            }
        }
    }
    ...
    public void draw(Graphics2D graph2) {
        if (isEditing) {
            ...
            if (sprite != null) {
                ...
                graph2.drawImage(sprite.image, (int) gameObject.transform.position.x, (int) gameObject.transform.position.y, (int) sprite.width, (int) sprite.height, null);
                alpha = 1.0f;
            ...
            wasDragged = true;
            graph2.setColor(new Color(1, 1, 1, 0.3f));
            ...
            graph2.fillRect((int)dragX, (int)dragY, (int)dragWidth, (int)dragHeight);
        }
    }
