...
public class Sprite extends Component {
    ...
    public int width, height, row, column, index;
    ...
    public Sprite(String pictureFile) {
        ...
        try {
            ...
            if (AssetPool.hasSprite(pictureFile)) {
                throw new Exception("Asset already: " + pictureFile);
            }
    ...
    public void draw(Graphics2D graph2) {
        ...
        transform.rotate(Math.toRadians(gameObject.transform.rotation), width * gameObject.transform.scale.x / 2.0, height * gameObject.transform.scale.y / 2.0);
        transform.scale(gameObject.transform.scale.x, gameObject.transform.scale.y);
        graph2.drawImage(image, transform, null);
    }
    ...
    public static Sprite deserialize() {
        ...
        if (isSubsprite) {
            ...
            if (!AssetPool.hasSpritesheet(filePath)) {
                System.out.println("Spritesheet '" + filePath + "' has not been loaded!");
                System.exit(-1);
            }
        ...
        if (!AssetPool.hasSprite(filePath)) {
            System.out.println("Sprite '" + filePath + "' has not been loaded!");
            System.exit(-1);
        }
        Parser.consumeEndObjectProperty();
